  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="end" className="w-[340px] p-0">
        ...
        {loading ? (
          <div className="px-3 py-4 text-xs text-muted-foreground">Ładowanie...</div>
        ) : notifications.length === 0 ? (
          <div className="px-3 py-4 text-xs text-muted-foreground">Brak powiadomień</div>
        ) : (
          <div className="max-h-[380px] overflow-y-auto py-1">
            {notifications.map((notification) => (
              <button
                ...
                className={`w-full text-left px-3 py-2 hover:bg-muted/60 transition-colors border-l-2 ${
                  notification.is_read ? 'border-transparent' : 'border-primary'
                }`}
              >
                <div className="flex items-start justify-between gap-3">
                  <p className="text-sm font-medium text-foreground line-clamp-2">{notification.title}</p>
