import { getTomorrowAt2359, toInputDateTime } from '@/lib/date-input';
...
export { getErrorMessage, getTomorrowAt2359, toInputDateTime };
...
export const normalizeOptions = (options: unknown): BetOption[] => {
  ...
  return options.map((option, index) => ({
    name:
      typeof (option as BetOption)?.name === 'string'
        ? (option as BetOption).name
        : `Opcja ${index + 1}`,
    odds:
      Number((option as BetOption)?.odds) > 0
        ? Number((option as BetOption).odds)
        : 1,
  }));
};

export const lockOptionsByType = (
  type: EditableBetType,
  current: BetOption[],
): BetOption[] => {
  if (type === 'single') {
...
export const lockEditableOptionsByType = (
  type: EditableBetType,
  current: EditableBetOption[],
): EditableBetOption[] => {
  if (type === 'single') {
    return [{ name: current[0]?.name || '1', odds: current[0]?.odds ?? '2' }];
  }
