...
export function useDailyJackpot() {
  ...
  const refreshBalanceForSettledPool = useCallback(
    async (nextSnapshot: DailyJackpotSnapshot) => {
      const shouldRefresh =
        (nextSnapshot.status === 'drawn' &&
          nextSnapshot.winnerUserId === profile?.id) ||
  ...
  const load = useCallback(
    async ({ silent = false }: { silent?: boolean } = {}) => {
      ...
      try {
        ...
        await refreshBalanceForSettledPool(nextSnapshot);
      } catch (error) {
        if (!silent) {
          const message =
