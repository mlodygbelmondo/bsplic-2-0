...
| 3 | Reset hasła | deep link, invalid/expired recovery, success | partial | Ekran przebudowany do parytetu PWA, obsługuje hash i PKCE; naked/invalid link pokazuje „Nieprawidłowy link”. Brak bezpiecznego aktywnego tokenu do wykonania sukcesu/expired. |
...
| 6 | Dolny dock | 5 tabów, selected, scroll-hide, safe area, casino tone | complete | 5 tabów, selected, safe area i casino tone potwierdzone na sportsbook, ruletce i blackjacku. Eksperymentalną regresję scroll-hide cofnięto w całości do zaakceptowanego checkpointu. |
...
| 8 | Notifications sheet | loading, unread/read, empty, sound, deep link | complete | Dane i unread, toggle dźwięku oraz przejście z powiadomienia do Jackpotu sprawdzone ręcznie; empty/loading/error mają jawne stany. |
...
| 12 | Sportsbook home | dark, light, loading, error, empty, data, pagination, realtime | complete | Dark/light i długi live dataset sprawdzone na iOS, scroll/paginacja i powrót z innych tras działają; istnieją jawne loading/error/empty oraz focus-gated realtime. |
...
| 14 | Bet card | compact/expanded, selection, closed/in-progress | partial | Karty danych i selection/kupon sprawdzone; w bieżącym datasetcie nie było reprezentatywnego zamkniętego i in-progress card jednocześnie do pełnego renderu. |
...
| 20 | Social composer | post/image, mentions, Eniu, draft persistence, offline, keyboard | partial | Composer, klawiatura, mention `@St…`, autocomplete i lokalna trwałość draftu sprawdzone. Upload/publikacja/Eniu nie były wykonywane tylko dla QA; offline blokuje zapis. |
