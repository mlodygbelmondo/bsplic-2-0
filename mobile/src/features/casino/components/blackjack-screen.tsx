...
import { AccessibilityInfo, Alert, ImageBackground, Pressable, ScrollView, Share, Text, View } from 'react-native';
...
function Hand({ cards, hiddenCount = 0 }: { cards: Card[]; hiddenCount?: number }) { return <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ width: '100%' }} contentContainerStyle={{ minHeight: 84, flexGrow: 1, justifyContent: 'center', gap: 6 }}>{cards.map((card, index) => <PlayingCard key={card.id ?? `${card.rank}-${card.suit}-${index}`} card={card} />)}{Array.from({ length: hiddenCount }, (_, index) => <PlayingCard key={`hidden-${index}`} hidden />)}</ScrollView>; }
...
export function BlackjackScreen() {
  ...
  return <ImageBackground source={require('../../../../assets/images/casino/blackjack-mobile-background.webp')} resizeMode="cover" style={{ flex: 1, backgroundColor: c.bg }}><ScrollView style={{ flex: 1 }} contentInsetAdjustmentBehavior="automatic" contentContainerStyle={{ padding: 16, paddingBottom: 116, gap: 12 }}>
    <View style={{ alignItems: 'center', paddingVertical: 10 }}><Text allowFontScaling={false} style={{ color: c.muted, fontSize: 11 }}>{tableLabel || 'Wczytywanie stołu…'}</Text></View>
    {game ? <AppCard style={{ minHeight: 390, backgroundColor: 'rgba(2,8,6,0.32)', borderColor: 'rgba(255,255,255,0.08)', borderRadius: 24, padding: 16, gap: 18 }}>
      <View style={{ alignItems: 'center', gap: 7 }}><Text allowFontScaling={false} style={{ color: c.text, backgroundColor: '#00000088', borderRadius: 999, paddingHorizontal: 12, paddingVertical: 5, fontSize: 11, fontWeight: '800' }}>Krupier {game.status !== 'playing' && game.status !== 'insurance' ? `· ${calculateHandValue(game.dealerHand)}` : ''}</Text><Hand cards={game.dealerHand} hiddenCount={game.dealerHiddenCount ?? 0} /></View>
      <View style={{ height: 1, backgroundColor: '#ffffff22' }} />
      {game.playerHands.map((hand, index) => <View key={hand.id} style={{ alignItems: 'center', gap: 7, borderRadius: 16, borderWidth: index === game.activeHandIndex && game.status === 'playing' ? 1 : 0, borderColor: c.gold, padding: index === game.activeHandIndex ? 8 : 0 }}><Text allowFontScaling={false} style={{ color: index === game.activeHandIndex ? c.gold : c.text, backgroundColor: '#00000088', borderRadius: 999, paddingHorizontal: 12, paddingVertical: 5, fontSize: 11, fontWeight: '800' }}>Twoja ręka {(game.playerHands.length ?? 0) > 1 ? index + 1 : ''} · {calculateHandValue(hand.cards)} · {hand.stake.toFixed(2)} zł</Text><Hand cards={hand.cards} /></View>)}
