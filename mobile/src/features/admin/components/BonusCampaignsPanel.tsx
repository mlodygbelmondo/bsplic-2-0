...
export function BonusCampaignsPanel() {
  ...
  return <View style={styles.stack}><AppCard style={styles.stack}><AppText variant="subtitle">Nowa kampania bonusowa</AppText><AppInput label="Tytuł" value={form.title} onChangeText={(title) => setForm({ ...form, title })} /><AppInput label="Opis" value={form.description} multiline style={styles.textarea} onChangeText={(description) => setForm({ ...form, description })} /><AppInput label="Kwota (zł)" keyboardType="decimal-pad" value={form.amount} onChangeText={(amount) => setForm({ ...form, amount })} /><AppInput label="Start (RRRR-MM-DD GG:MM)" value={form.startsAt.replace('T', ' ')} onChangeText={(startsAt) => setForm({ ...form, startsAt: startsAt.replace(' ', 'T') })} /><AppInput label="Wygaśnięcie (RRRR-MM-DD GG:MM)" value={form.expiresAt.replace('T', ' ')} onChangeText={(expiresAt) => setForm({ ...form, expiresAt: expiresAt.replace(' ', 'T') })} /><AppButton loading={busy === 'create'} onPress={create}>Utwórz kampanię</AppButton></AppCard><AdminState empty={campaigns.length === 0} emptyTitle="Brak kampanii bonusowych." />{campaigns.map((item) => { const status = getBonusCampaignAdminStatus(item); const canDeactivate = item.is_active && (status === 'active' || status === 'scheduled'); return <AppCard key={item.id} style={styles.stack}><View style={styles.row}><View style={styles.flex}><AppText variant="label">{item.title}</AppText><AppText variant="caption" tone="muted">{item.description}</AppText></View><AppBadge label={LABELS[status]} tone={status === 'active' ? 'success' : status === 'scheduled' ? 'primary' : 'neutral'} /></View><AppText>{formatBonusAmount(item.amount)} · odebrano {item.claim_count}</AppText><AppText variant="caption" tone="muted">{new Date(item.starts_at).toLocaleString('pl-PL')} – {new Date(item.expires_at).toLocaleString('pl-PL')}</AppText>{canDeactivate ? <AppButton variant="outline" loading={busy === item.id} onPress={() => deactivate(item)}>Wyłącz kampanię</AppButton> : null}</AppCard>; })}</View>;
}
