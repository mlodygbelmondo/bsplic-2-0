-- Longer bonus with smaller individual payouts, plus a free-only endpoint for autoplay.
ALTER TABLE public.casino_slot_bonus DROP CONSTRAINT casino_slot_bonus_remaining_check;
ALTER TABLE public.casino_slot_bonus ADD CONSTRAINT casino_slot_bonus_remaining_check CHECK (remaining BETWEEN 0 AND 15);
...
BEGIN
 ...
 IF p_request_id IS NULL OR p_stake IS NULL OR p_stake::text IN ('NaN','Infinity','-Infinity') OR p_stake < 1 OR p_stake <> round(p_stake,2) THEN RAISE EXCEPTION 'INVALID_STAKE'; END IF;
 ...
 IF free_left=0 AND scatters>=4 THEN awarded:=15; END IF;
 FOR cascade IN 0..11 LOOP
  ...
  FOR g IN SELECT value FROM jsonb_array_elements(groups) LOOP
   ...
   base := CASE p_game WHEN 'bandit' THEN 2.5654 WHEN 'candy' THEN 0.2111 WHEN 'ember' THEN 2.532 WHEN 'tide' THEN 0.2108 END;
  ...
  END LOOP;
  -- Fifteen smaller free spins: 15 * 0.75 = 11.25 standard spins in expectation.
  IF charged=0 THEN frame_payout:=round(frame_payout*0.75,2); END IF;
...
-- Serialize with all wallet actions. A stale bonus request must never debit a paid stake.
CREATE OR REPLACE FUNCTION public.casino_slot_bonus_spin(p_game text, p_stake numeric, p_request_id uuid) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE u uuid:=auth.uid(); existing public.casino_slot_spins;
BEGIN
 IF u IS NULL THEN RAISE EXCEPTION 'AUTH_REQUIRED'; END IF;
 ...
 SELECT * INTO existing FROM public.casino_slot_spins WHERE id=p_request_id;
 IF FOUND THEN
  IF existing.user_id<>u OR existing.charged<>0 THEN RAISE EXCEPTION 'REQUEST_CONFLICT'; END IF;
  RETURN public.casino_slot_spin(p_game,p_stake,p_request_id);
 END IF;
 IF NOT EXISTS (SELECT 1 FROM public.casino_slot_bonus WHERE user_id=u AND game=p_game AND remaining>0) THEN
  RAISE EXCEPTION 'BONUS_FINISHED';
 END IF;
 RETURN public.casino_slot_spin(p_game,p_stake,p_request_id);
END; $$;
REVOKE ALL ON FUNCTION public.casino_slot_bonus_spin(text,numeric,uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.casino_slot_bonus_spin(text,numeric,uuid) TO authenticated;
