-- Add end date to proposals so admin editor can be prefilled correctly.
ALTER TABLE public.bet_proposals
ADD COLUMN IF NOT EXISTS ends_at TIMESTAMPTZ;

-- Server-side balance credit for admin settlement.
...
BEGIN
  ...
  IF p_amount IS NULL OR p_amount <= 0 THEN
    RAISE EXCEPTION 'Kwota musi być większa od 0';
  END IF;
