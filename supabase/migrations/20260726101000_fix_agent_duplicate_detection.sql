-- Fix agent duplicate detection.
--
-- The previous rule skipped a candidate when the bet type matched AND
-- (normalised title matched OR option signature matched) inside a +/-24h window.
-- `private.agent_option_signature` is the sorted list of option NAMES only, so
-- every `1x2` market named 1/X/2, every Over/Under and every Tak/Nie market
-- collided with every other one. On a tournament day with eight fixtures, seven
-- were rejected as "approximate duplicate".
--
-- New rule: an exact `agent_duplicate_key` hit is the primary deterministic
-- guard; the fuzzy fallback now requires title AND signature AND a tight +/-6h
-- window.
--
-- `public.agent_create_bets` is rewritten separately in
-- 20260726103000_agent_create_bets_with_ako.sql and uses the same helper.

CREATE OR REPLACE FUNCTION private.agent_find_duplicate(
  p_bet_type TEXT,
  p_title_norm TEXT,
  p_option_signature TEXT,
  p_ends_at TIMESTAMPTZ,
  p_agent_duplicate_key TEXT DEFAULT NULL,
...
AS $$
BEGIN
  IF p_agent_duplicate_key IS NOT NULL THEN
    IF EXISTS (
      SELECT 1
      FROM public.bets b
      WHERE b.agent_duplicate_key = p_agent_duplicate_key
    ) THEN
      RETURN 'duplicate agent_duplicate_key on existing bet';
    END IF;

    IF EXISTS (
      SELECT 1
      FROM public.bet_proposals p
      WHERE p.agent_duplicate_key = p_agent_duplicate_key
        AND p.status = 'pending'
        AND (p_exclude_proposal_id IS NULL OR p.id <> p_exclude_proposal_id)
    ) THEN
      RETURN 'duplicate agent_duplicate_key on pending proposal';
    END IF;
  END IF;
  ...
  IF EXISTS (
    SELECT 1
    FROM public.bets b
    WHERE b.bet_type = p_bet_type
      AND COALESCE(private.agent_normalize_text(b.title), '') = p_title_norm
      AND private.agent_option_signature(b.options) = p_option_signature
      AND b.ends_at IS NOT NULL
      AND b.ends_at BETWEEN (p_ends_at - INTERVAL '6 hours') AND (p_ends_at + INTERVAL '6 hours')
  ) THEN
    RETURN 'approximate duplicate against existing bet';
  END IF;
  ...
  IF EXISTS (
    ...
    WHERE p.status = 'pending'
      AND p.bet_type = p_bet_type
      AND COALESCE(private.agent_normalize_text(p.title), '') = p_title_norm
      AND private.agent_option_signature(p.options) = p_option_signature
      AND p.ends_at IS NOT NULL
      AND p.ends_at BETWEEN (p_ends_at - INTERVAL '6 hours') AND (p_ends_at + INTERVAL '6 hours')
      AND (p_exclude_proposal_id IS NULL OR p.id <> p_exclude_proposal_id)
  ) THEN
    RETURN 'approximate duplicate against pending proposals';
  END IF;
...
REVOKE ALL ON FUNCTION private.agent_find_duplicate(TEXT, TEXT, TEXT, TIMESTAMPTZ, TEXT, UUID) FROM PUBLIC;
...
BEGIN
  ...
  LOOP
    ...
    IF v_agent_duplicate_key IS NOT NULL AND v_agent_duplicate_key = ANY(v_seen_keys) THEN
      v_skipped := v_skipped || jsonb_build_array(jsonb_build_object(
        'title', v_title,
...
-- Accepting a proposal now uses the same duplicate rule and carries agent
-- provenance onto the published bet.
...
BEGIN
  ...
  LOOP
    ...
    v_duplicate_reason := private.agent_find_duplicate(
      ...
      v_option_signature,
      v_proposal.ends_at,
      v_proposal.agent_duplicate_key,
      v_proposal.id
    );
    ...
    v_event_key := NULLIF(BTRIM(COALESCE(v_proposal.agent_metadata, '{}'::JSONB) ->> 'event_key'), '');
    ...
    BEGIN
      ...
      VALUES (
        ...
        v_proposal.agent_duplicate_key,
        COALESCE(v_proposal.agent_metadata, '{}'::JSONB),
        v_event_key
      )
