-- Two additional slot mechanics and disclosed, player-favouring paid-spin odds.
ALTER TABLE public.casino_slot_bonus DROP CONSTRAINT casino_slot_bonus_game_check;
ALTER TABLE public.casino_slot_bonus ADD CONSTRAINT casino_slot_bonus_game_check CHECK (game IN ('bandit','candy','ember','tide'));
ALTER TABLE public.casino_slot_spins DROP CONSTRAINT IF EXISTS casino_slot_spins_game_check;
ALTER TABLE public.casino_slot_spins ADD CONSTRAINT casino_slot_spins_game_check CHECK (game IN ('bandit','candy','ember','tide'));
...
BEGIN
 ...
 -- Paid spins seed a profitable group with probability 90% during boost, 60% otherwise.
 IF free_left=0 AND public._slot_random()<(CASE WHEN boosted THEN 0.90 ELSE 0.60 END) THEN
  symbol := floor(public._slot_random()*7)::integer;
  IF p_game IN ('bandit','ember') THEN
   i := floor(public._slot_random()*4)::integer*6+1;
   FOREACH j IN ARRAY ARRAY[i,i+1,i+2,i+6,i+7] LOOP board[j]:=symbol; END LOOP;
  ELSE
   SELECT array_agg(position) INTO survivors FROM generate_series(1,30) position;
   FOR col IN 1..13 LOOP
    j:=1+floor(public._slot_random()*cardinality(survivors))::integer;
    board[survivors[j]]:=symbol;
    survivors:=array_remove(survivors,survivors[j]);
   END LOOP;
  END IF;
 END IF;
