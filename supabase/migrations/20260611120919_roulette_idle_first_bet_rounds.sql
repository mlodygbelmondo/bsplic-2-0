-- Run the shared roulette table only when players are actually betting.
--
-- A missing current_round now means the table is idle. The first accepted bet
-- creates the next waiting round and starts its betting timer.

CREATE INDEX IF NOT EXISTS casino_roulette_rounds_active_table_round_idx
  ON public.casino_roulette_rounds (table_key, round_number DESC)
...
BEGIN
  ...
  IF NOT FOUND THEN
    IF EXISTS (
      ...
      RAISE EXCEPTION 'Koło już się kręci. Poczekaj na następną rundę.';
    END IF;
...
REVOKE EXECUTE ON FUNCTION public.place_roulette_table_bet(UUID, TEXT, TEXT, NUMERIC, TEXT) FROM PUBLIC, anon;
...
GRANT EXECUTE ON FUNCTION public.place_roulette_table_bet(UUID, TEXT, TEXT, NUMERIC, TEXT) TO authenticated;
