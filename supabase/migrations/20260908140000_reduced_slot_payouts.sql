-- Lower ordinary payouts by 15.38% for clusters and 14.29% for pay-anywhere.
-- Lucky shot, random symbol draws, free-spin awards and caps stay the same.
...
BEGIN
 ...
 FOR cascade IN 0..11 LOOP
  ...
  FOR g IN SELECT value FROM jsonb_array_elements(groups) LOOP
   ...
   base := CASE WHEN p_game IN ('bandit','ember') THEN 2.20 ELSE 0.18 END;
