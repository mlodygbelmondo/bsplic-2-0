 ...
 FOREACH v_game IN ARRAY ARRAY['bandit','candy','ember','tide'] LOOP
  FOREACH stake IN ARRAY ARRAY[1,5,100] LOOP
   FOREACH symbol IN ARRAY ARRAY[0,6] LOOP
    ...
    expected:=round(stake*(CASE WHEN v_game IN ('bandit','ember') THEN 2.2 ELSE 0.18 END)*(1+symbol*0.25),2)*(CASE WHEN v_game='candy' THEN 5 WHEN v_game='tide' THEN 3 ELSE 1 END);
  ...
  END LOOP;
 END LOOP;
 RAISE NOTICE 'PASS: reduced payout tables, rounding, multipliers and net across all games';
END; $$;
ROLLBACK;
